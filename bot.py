...
def апи_обновить_остаток(название: str, изменение: int) -> dict:
    """
    POST /update-stock.
    Сериализуем вручную через json.dumps + encode, чтобы избежать
    конфликта заголовков при использовании json= и headers= одновременно.
    """
...
def записать_операцию(д: dict, сайт_данные: dict) -> None:
    ...
    Структура СКЛАД:   A:UID | B:Дата | C:Операция | D:Поставка | E:Товар | F:Приход | G:Расход | H:Остаток(формула)
    Структура ФИНАНСЫ: A:UID | B:Дата | C:Операция | D:Поставка | E:Товар | F:Сумма   | G:Комментарий
    Структура ИСТОРИЯ: A:UID | B:Дата | C:Операция | D:Товар    | E:Цена  | F:Кол-во | G:Поставка | H:Клиент | I:Прибыль

    В столбце H листа СКЛАД стоит формула — передаём пустую строку чтобы не затирать.
    """
    сег  = datetime.now().strftime("%d.%m.%Y")
    uid  = д["uid"]
    опер = д["операция"]
    тов  = д["товар"]   # уже каноническое имя с сайта
    кол  = д["количество"]
    ...
    # СКЛАД: A:UID | B:Дата | C:Операция | D:Поставка | E:Товар | F:Приход | G:Расход
    # Передаём только 7 столбцов — H (формула остатка) не трогаем
    if опер == "Продажа":
...
async def обработать_сообщение(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not рез_сайт.get("success"):
        ответ += f"\n⚠️ Сайт не обновлён: {рез_сайт.get('error', '?')} | товар запроса: {данные['товар']}"
